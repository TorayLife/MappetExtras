...
Исправлен метод getAngle для возвращения значений в градусах, удалены интерфейсы IMinecraftHUD, IMinecraftArmRender, IMinecraftCamera, IMinecraftCameraShake и связанные с ними методы, а также добавлена настройка "Текст по умолчанию".
Также удалена вкладка "Измерения".
...
* Изменения в API:
    ...
    * Удалена вкладка "Измерения" до лучших времён.
* Нововведения:
...
В этой версии был исправлен краш при создании измерений на сервере и двойной вызов некоторых триггеров.

### Полный список изменений:

* Исправления:
    * Исправлена ошибка из-за которой триггеры при тике игрока вызывались дважды за тик
    * Исправлена ошибка из-за которой в некоторых триггерах был недоступен `c.getValue('event')`
    * Исправлен краш при создании измерения на сервере
    * Исправлена регистрация измерения на сервере


## Версия 1.4.1

В этой версии был исправлен метод для воспроизведения анимации у Chameleon морфов.

### Полный список изменений:

* Исправления:
  * Исправлена работа метода `IScriptEntity.playAnimation(String animation)`.

    
## Версия 1.4.0

В этом обновлении добавлены кастомные измерения и возможность воспроизводить Chameleon анимации у НПС и игроков.

### Полный список изменений:

* Новые возможности:
  * Добавлена вкладка "Измерения", где можно создавать свои кастомные измерения
  * Добавлены новые глобальные триггеры:
    * Игрок: изменение измерения
* Изменения в API:
  * Добавлен метод `IScriptWorld.delete()`, который удаляет папку измерения (не работает со стандартными измерениями Майнкрафта)
  * Добавлен метод `IScriptServer.getRegisteredDimensions()`, который возвращает список зарегистрированных измерений
  * Добавлен метод `IScriptEntity.playAnimation(String animation)`, который воспроизводит анимацию у Chameleon морфа
* Исправления:
  * Исправлен метод `IScriptEntity.getEntityId()`, теперь он снова возвращает строку
* Прочее:
  * Добавлены текстуры для инструмента на день рождения (день релиза) Маппета - 13 сентября
